...
- 路径支持中文和英文字母, 数字, 空格, 点, 下划线, 连字符, 半角逗号 `,` 和括号 `()`, 例如 `代数/模(A,B).typ`; 使用 `/` 分隔文件夹. `new:note` 会自动创建父目录, 不会覆盖已有文件.
...
各行格数须为正整数且不递增; `labels` 可省略, 填写时按逐行顺序放入各格, 数量须等于总格数. 方格会根据标签大小自动扩展. 单格填字写作 `#young(1, labels: (1,))`. 多幅图可使用 `#diagram-row[...]` 并排. Young 图与交换图共用下述增量编译和磁盘缓存.
