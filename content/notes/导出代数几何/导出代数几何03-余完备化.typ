...
#definition(title:[筛滤范畴 / 滤过范畴])[
  设 $cal(C)$ 是一个 1-范畴, 那么:

  + 我们称 $cal(C)$ 是 *1-筛滤的*, 当且仅当以 $cal(C)$ 为指标在 $Set$ 中的余极限与有限积交换.
  + 我们称 $cal(C)$ 是 *滤过的*, 当且仅当以 $cal(C)$ 为指标在 $Set$ 中的余极限与有限极限交换.
]

#proposition(title:[筛滤的等价判别])[
  一个 1-范畴是 1-筛滤的当且仅当其非空, 且对角函子 $Delta:cal(C)->cal(C) times cal(C)$ 是*余终的*. 一个函子 $u:cal(C)->cal(D)$ 余终的含义是, 对任何 $F:cal(D)->cal(E)$, 都有自然的比较映射是等价
...
#proof[
  ...
  因此若 $Delta:cal(C)->cal(C)^2$ 是余终的, 恰好说明了上述等式成立.

  反过来则需要一个小论证, 取
  $ F_1 (c) = Hom(c_1,c), quad F_2 (c) = Hom(c_2,c) $
  筛滤条件给出
  $ colim_c (Hom(c_1,c) times Hom(c_2,c)) tilde.eq * $
  而左边恰是逗号范畴 $(c_1,c_2)\/Delta$ 的连通分支集合, 于是 $Delta$ 余终.
]
...
#definition(title:[筛滤完备化 / ind-完备化])[
  设 $cal(C)$ 是一个小 1-范畴, 我们定义:

  + 若 $cal(C)$ 具有有限余极限, 令 $PSh_Sigma (cal(C))^suit.heart subset PSh(cal(C))^suit.heart$ 是由保持有限积的 $cal(C)^opp->Set$ 函子张成的全子范畴, 称之为 $cal(C)$ 的 *1-筛滤完备化* (1-sifted completion).
...
#remark[
  显然若 $cal(C)$ 具有有限余极限, 则 $Ind(cal(C)) subset PSh_Sigma (cal(C))^suit.heart$.
]
...
我们可以用很显然的方式推广筛滤和滤过范畴的定义

#definition(title:[筛滤范畴 / 滤过范畴])[
  设 $cal(C)$ 是一个 $oo$-范畴, 那么:

  + 我们称 $cal(C)$ 是 *筛滤的*, 当且仅当以 $cal(C)$ 为指标在 $Ani$ 中的余极限与有限积交换.
...
#definition(title:[筛滤完备化 / ind-完备化])[
  ...
  + 若 $cal(C)$ 具有有限余极限, 令 $PSh_Sigma (cal(C)) subset PSh(cal(C))$ 是由保持有限积的 $cal(C)^opp->Ani$ 函子张成的全子范畴, 称之为 $cal(C)$ 的 *筛滤完备化* (1-sifted completion).
  + 若 $cal(C)$ 具有有限余极限, 令 $Ind(cal(C)) subset PSh(cal(C))$ 是由保持有限极限的 $cal(C)^opp->Ani$ 函子张成的全子范畴, 称之为 $cal(C)$ 的 *ind-完备化* (ind-completion).
]

我们承认以下关于筛滤完备化和 Ind-完备化的结论, 具体证明可以参考 Lurie 的 Higher Topos Theory, 第5章:

#proposition[
  设 $cal(C)$ 是一个小的 $oo$-范畴, 且 $cal(C)$ 具有有限的余积, 那么:

  + 包含函子 $i: PSh_Sigma (cal(C)) arrow.hook PSh(cal(C))$ 保持筛滤的余极限, 并且存在左伴随, 从而 $PSh_Sigma (cal(C))$ 是可呈示的.
  + Yoneda 函子 $yo : cal(C) -> PSh(cal(C))$ 落在 $PSh_Sigma (cal(C))$ 中.
  + 一个对象 $X in PSh(cal(C))$ 落在 $PSh_Sigma (cal(C))$ 中当且仅当存在可表作 $yo:cal(C) arrow.hook PSh(cal(C))$ 中对象的滤过余极限的两个对象 $X_0$ 和 $X_1$ (i.e. $X_0,X_1 in Ind(cal(C))$), 以及*反身的*平行态射, 即存在
    $ X_1 arrows^(d_0)_(d_1) X_0, quad s:X_0->X_1 $
    满足 $d_0 s = d_1 s = id_(X_0)$, 使得
    $ X tilde.eq "coEq"(X_1 arrows^(d_0)_(d_1) X_0) $
]

#proposition[
  设 $cal(C)$ 是一个小的 $oo$-范畴, 且 $cal(C)$ 具有有限的余积, 那么:

  + 若 $cal(D)$ 是具有 1-筛滤余极限的 1-范畴, 则沿 Yoneda 的限制诱导等价
    $ yo^* : Fun^("sift") (PSh_Sigma (cal(C)), cal(D)) -->^~ Fun(cal(C),cal(D)) $
    其中 $Fun^("sift") (PSh_Sigma (cal(C)),cal(D)) subset Fun(PSh_Sigma (cal(C)),cal(D))$ 是保持 1-筛滤余极限函子的全子范畴.
  + 若 $cal(D)$ 是具有全部余极限的 1-范畴, 则沿 Yoneda 的限制诱导等价
  $ yo^* : Fun'(PSh_Sigma (cal(C)), cal(D)) -->^~ Fun^union.sq (cal(C),cal(D)) $
...
#proposition[
  ...
  + 若 $cal(D)$ 是具有滤过余极限的 1-范畴, 则沿 Yoneda 的限制诱导等价
    $ yo^* : Fun^("filt") (PSh_Sigma (cal(C)), cal(D)) -->^~ Fun(cal(C),cal(D)) $
    其中 $Fun^("filt") (PSh_Sigma (cal(C)),cal(D)) subset Fun(PSh_Sigma (cal(C)),cal(D))$ 是保持滤过余极限函子的全子范畴.
  + 若 $cal(D)$ 是具有全部余极限的 1-范畴, 则沿 Yoneda 的限制诱导等价
  $ yo^* : Fun'(PSh_Sigma (cal(C)), cal(D)) -->^~ Fun^"rex" (cal(C),cal(D)) $
...
类似地我们可以用典范的方法推广 $kappa$-筛滤的概念, 读者可自行完成.
