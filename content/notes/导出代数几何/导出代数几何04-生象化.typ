...
#definition(title:[投射对象])[
  ...
  $ abs(Map_(cal(C))(X,Y_bullet)) simeq Map_(cal(C)) (X,abs(Y_bullet)) $
  则称之为 $cal(C)$ 的一个*投射对象*.
]
...
#remark(title:[1-投射对象])[
  ...
  $ Map_(cal(C))(X,"coEq"(X_1 arrows^(d_0)_(d_1) X_0)) simeq "coEq"(Map_(cal(C))(X,X_1) arrows^(d'_0)_(d'_1) Map_(cal(C))(X,X_0)) $
  则称之为 *1-投射对象*.
]
...
#remark[
  ...
  $ colim_([n] in Delta^opp)^Set K_n simeq pi_0 (S^1) simeq * $
  因此先是在 $Set$ 中取到余极限, 再映到生象, 得到
  $ Map(*,colim^(Set)_(Delta^opp) K) simeq * $
  但如果先映到 $Ani$ 再取同一个筛极限
  $ colim_([n]in Delta^opp)^Ani Map(*,K_n) simeq |K| simeq S^1 $
  二者显然不同, 核心问题就是通过离散嵌入
...
$ Fun^times ((cal(C)^(omega"p"))^opp,Set) simeq "1-"sInd(cal(C)^(omega"p")) simeq cal(C) $

左边表示保持有限乘积函子的全子范畴. 我们固然希望在 $oo$-范畴中推广这一概念:

#definition(title:[生象化])[
  设 $cal(C)$ 是一个紧 1-投射生成的范畴, 则 $cal(C)$ 的*生象化* (animation) 定义为
  $ Ani(cal(C)) simeq sInd(cal(C)^(omega"p")) simeq Fun^times ((cal(C)^(omega"p"))^opp,Ani) $ 
  其中的对象称之为 $cal(C)$ 的*生象对象* (animated objects).
]

#lemma[
  设 $cal(C)$ 是一个紧 1-投射生成的范畴, 则 $Ani(cal(C))$ 是紧投射生成的范畴, 且 $Ani(cal(C))^(omega"p") simeq cal(C)^(omega 1"p")$.
]

#remark(title:[生象化的泛性质])[
  若 $cal(D)$ 具有筛余极限, 那么限制到紧投射生成元给出等价
  $ Fun^"sift" (Ani(cal(C)),cal(D)) simeq Fun(cal(C)^(omega"p"),cal(D)) $
  左边代表全体保持筛余极限的函子, 也就是说, 任意函子
  $ f:cal(C)^(omega"p") -> cal(D) $
  都在可缩选择下存在唯一的保持筛余极限的延拓
  $ tilde(f):Ani(cal(C)) -> cal(D) $
  使得
  $ tilde(f) simeq Lan_(yo) f $
  若 $cal(D)$ 甚至具有所有的余极限, 那么
  $ Fun^"L" (Ani(cal(C)),cal(D)) simeq Fun^union.sq (cal(C)^(omega"p"),cal(D)) $
...
#example[
  注意到对任意集合 $X$, 都有
  $ X simeq product.co_(x in X) * $
  而 $*$ 显然是紧 1-投射对象, 于是 $Set$ 是紧 1-投射生成的. $Set$ 中的紧 1-投射对象恰好是有限集合. 普通的 $Set$ 可以通过这些小生成元恢复, 即
  $ Set simeq "1-"sInd(cat("FinSet")) $
  换成生象化就是
  $ Ani simeq Ani(Set) simeq sInd(cat("FinSet")) $
]

#example(title:[环的模范畴])[
  设 $R$ 是一个结合环, (左) $R$-模范畴 $Mod_R$ 满足
  $ Mod_R^(omega"p") simeq {"有限生成的投射左" R"-模"} $
  并且 $Mod_R = "1-"sInd(Mod_R^(omega"p"))$, 那么可以定义
  $ Ani(Mod_R) simeq sInd(Mod^(omega"p")_R) $
...
#example[
  ...
  $ Ani(Alg) simeq simp(Alg)[W^(-1)] simeq dga_(>=0)["qis"^(-1)] $
]
...
$ Fun^times (cal(C)^opp,sSet) simeq sInd(cal(C)) $
