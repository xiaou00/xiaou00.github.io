...
$ dif : A -> Omega^1_(A/k) $
控制, 且有
$ Der_k (A,M) simeq Hom_A (Omega^1_(A/k),M) $
这确实完美地表示了普通的平方零扩张
...
展开
$ simeq A plus.o M[n] plus.o M[n] plus.o (M[n] times.o_A M[n]) $
规定四个部分
$
...
$ mu : B times.o_A B -> B $
展开得
$ B times.o_A B simeq A plus.o M plus.o M plus.o (M times.o_A M) $
定义
$ (a,x,y,z) |-> (a,x+y) $
...
#proof[
  ...
  $ U(colim_i (A plus.o M_i)) &simeq colim_i (U(A) plus.o V(M_i)) \
  &simeq (colim_i Delta U(A)) plus.o colim_i V(M_i) $
  筛范畴是弱可缩 (所有态射变成等价后可缩) 的, 有
  $ colim_i Delta U(A) simeq U(A) $
  又因为这个筛余极限被 $V$ 保持
  $ colim_i V(M_i) simeq V(colim_i M_i) $
  从而
  $ U(colim_i SqZ_A (M_i)) simeq U(A) plus.o V(colim_i M_i) = U(SqZ_A (colim_i M_i)) $
  由于 $U$ 保守, 比较映射是等价, 从而 $SqZ_A$ 保持筛余极限.

  下面证明其保持极限, 对任意图
  $ X : J -> cal(C)_("/"A) $
  遗忘到 $cal(C)$ 的图依然为 $X$, 且有
  $ lim_(cal(C)_("/"A)) X simeq A times_(lim_J Delta A) lim_J X quad (*) $
  令
  $ X_j = SqZ_A (M_j) = A plus.o M_j $
  设 $M := lim_J M_j$ 是在 $Mod^(>=0)_A$ 中的极限, 由 $U$ 保持极限, 我们有
  $ U(lim_J (A plus.o M_j)) simeq lim_J (U(A) plus.o V(M_j)) $
  而在生象模中, $plus.o$ 是有限双积, 极限函子保持它
  $ lim_J (Delta U(M) plus.o V(M_j)) simeq (lim_J Delta U(A)) plus.o (lim_J V(M_j)) $
  带入 $(*)$ 得到
  $ U(lim_(cal(C)_("/"A)) SqZ_A (M_j)) &simeq U(A) times_(lim_J Delta U(A)) [(lim_J Delta U(A)) plus.o lim_J V(M_j)] \ &simeq U(A) plus.o lim_J V(M_j) $
  再由 $V$ 保持极限有
  $ lim_J V(M_j) simeq V(lim_J M_j) = V(M) $
  于是
  $ U(lim_J SqZ_A (M_j)) simeq U(A) plus.o V(M) = U(SqZ_A (M)) $
  因此比较映射
...
$ Der_k (A,M) simeq Hom_((CAlg_k)_("/"A))(A,A plus.o M) $
...
亦即 $epsilon compose s simeq id$. 换句话说, 导子就是平方零扩张投影 $A plus.o M -> A$ 的一个截面.
...
#proof[
  ...
  $ U (A plus.o M) simeq U(A) plus.o M $
  并且
  $ U(epsilon) : U(A) plus.o M -> U(A) $
  是第一分量投影, 考察
  $ U(s) : U(A) -> U(A) plus.o M $
  满足 $pr_A compose U(s) simeq id_(U(A))$, 那取第二分量投影就得到了
  $ d_s := pr_M compose U(s) : U(A) -> M $
  即 $U(s) simeq (id_A,d_s)$.
]
...
而因为 $R$ 是终对象, $Map_(cal(C))(R,R)simeq *$, 于是有
$ Der_k (R,Sigma^n M) simeq Omega Der_k (R,Sigma^(n+1)M) $
