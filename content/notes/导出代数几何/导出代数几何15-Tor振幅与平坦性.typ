...
#remark[
  ...
  $ pi_i (M) = 0, quad i>0 $
  例如当 $M=A$ 时, 显然 $A times.o_A N tilde.eq N$ 对任意离散 $N$ 都离散, 从而 $Amp(A)=[0,0]$, 但 $A$ 自身完全可以有 $pi_i A!=0$. Tor 振幅的信息是相对某个基环刻画的.
]
...
#proof[
  ...
  从而
  $ M times.o_A (pi_i X[i]) tilde.eq (M times.o_A pi_i X)[i] in Mod^[a+i,b+i]_A $
...
#proof[
  ...
  $ ... ->F_1->F_0->pi_*M->0 $
  其中每个
  $ F_p tilde.eq plus.o.big_alpha R[d_(p,alpha)] $
  都是分次 $R$-模. (p.s. 这里的方括号含义是位移) 并且有 $R[d] = pi_* (A[d])$ 显然, 于是每个 $F_p$ 都实现为自由 $A$-模
  $ P_p := plus.o.big_alpha A[d_(p,alpha)], quad pi_*P_p tilde.eq F_p $
  易证其构成一个单纯增广模
  $ P_bullet -> M $
  满足 
  $ abs(P_bullet) tilde.eq M $
  且
  $ pi_* P_bullet -> pi_* M $
  是单纯自由解消, 作用 $- times.o_A N$ 有
  $ M times.o_A N tilde.eq abs(P_bullet) times.o_A N tilde.eq abs(P_bullet times.o_A N) $
  ...
  则
  $ P_p times.o_A N tilde.eq plus.o.big_alpha N[d_alpha] $
  从而
  $ pi_*(P_p times.o_A N) tilde.eq pi_*P_p times.o_R pi_*N $
  于是
  $ E^1_(p,*) tilde.eq F_p times.o_R pi_*N $
  ...
  另一方面由于 $abs(X_bullet) tilde.eq M times.o_A N$, 谱序列收敛到
...
#proposition[
  ...
  $ Tor^A_n (M,X) = pi_n (M times.o_A X) tilde.eq pi_0 M times.o_(pi_0 A) pi_n X $
]
...
#proof[
  ...
  $ pi_0 (X times.o_A pi_i X) tilde.eq pi_0 M times.o_(pi_0 A) pi_i X $
  从而这一层只在次数 $i$ 有同伦群, 且同伦群为 $pi_0 M times.o_(pi_0 A) pi_i X$. 对 Postnikov 塔归纳即证.
]

从而, 作为分次模, 有
$ pi_* M tilde.eq pi_* A times.o_(pi_0 A) pi_0 M $
...
#proof[
  假设 $M$ 平坦, 第一个条件显然, 第二个条件由前一个命题, 由 $M times.o_A A tilde.eq M$, 有
  $ pi_n M tilde.eq pi_0 M times.o_(pi_0 A) pi_n A $
  反过来比较复杂, 重新改写条件无非就是分次 $pi_*A$-模同构
  $ pi_*M tilde.eq pi_*A times.o_(pi_0 A) pi_0 M $
  由于 $pi_0 M$ 是离散的 $pi_0 A$-模, $pi_*A times.o_(pi_0 A)pi_0 M$ 是平坦的分次 $pi_*A$-模, 有对任意分次 $pi_*A$-模 $P$, 有
  $ (pi_*A times.o_(pi_0 A)pi_0M) times.o_(pi_* A) P tilde.eq pi_0 M times.o_(pi_0 A) P $
  ...
  而
  $ pi_* M times.o_(pi_* A) N tilde.eq (pi_*A times.o_(pi_0) pi_0 M) times.o_(pi_*A) N tilde.eq pi_0 M times.o_(pi_0 A) N $
  都集中在内部次数 $0$, 故谱序列坍缩, 得到
  $ pi_n(M times.o_A N) = 0, quad (n!=0) $
  以及
  $ pi_0 (M times.o_A N) tilde.eq pi_0 M times.o_(pi_0 A) N $
